...
function getRectangleString(width, height) {
    ...
    for(let i = 0; i < 3; i++){
        let lines_repeat = 1;
        if (i == 1) {
            start = end = '\u2502'; inside = ' '; lines_repeat = height - 2;
        }
        if (i == 2) { start = '\u2514'; inside = '\u2500'; end = '\u2518'; }
        result += `${start}${inside.repeat(width-2)}${end}\n`.repeat(lines_repeat);
    }
    return result;
}
 ...
 *
 */
 function getEncrypthedCharCode(charCode) {
     if (charCode > 109) return charCode - 13;
     if (charCode > 77 && charCode < 91) return charCode - 13;
     return charCode + 13;
 }
 ...
 function encodeToRot13(str) {
     ...
     for(let i = 0; i < str.length; i++){
         ...
         if (char.search(/[A-Za-z]/) >= 0) {
             let encrCharCode = getEncrypthedCharCode(char.charCodeAt(0));
             result += String.fromCharCode(encrCharCode);
             continue;
         }
         result += char;
     }
 ...
 */
 function getCardId(value) {
     if (value.search('10') >= 0) value = value.replace('10', 'T');
     let columnC = 'A23456789TJQK', rowC = '♣♦♥♠';
     return rowC.search(value[value.length-1]) * 13 + columnC.search(value[0]);
 }
